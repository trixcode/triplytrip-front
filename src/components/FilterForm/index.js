...
const checkBoxOpen = () => {
	const checkboxWrapper = document.querySelectorAll('.filter-form-checkboxs__wrapper');
	const icon = document.querySelectorAll('.filter-form-checkboxs__title__icon');
	if (bool) {
		icon[0].classList.remove('action')
		icon[1].classList.add('action')
		checkboxWrapper[0].classList.remove('checkboxs_hide')
		bool = false;
	} else {
		icon[1].classList.remove('action')
		icon[0].classList.add('action')
		checkboxWrapper[0].classList.add('checkboxs_hide')
		bool = true;
	}
}
		...
		<div className='filter-form'>
			<div className='container'>
				...
				<div className='filter-form-checkboxs'>
					...
					<form
						className='filter-form-checkboxs__wrapper checkboxs_hide'>
						{tagsName.map(key => {
							return (
								<div
									key={key}
